...
describe("Dead Man's Switch Contract", function () {
  async function deploySednFixture() {
    ...
    const sedn = await sednFactory.connect(owner).deploy(mockV3Aggregator.address);
    await sedn.deployed();
    return { sedn, owner, addr1, addr2 };
  }
  describe("Switch Creation", function () {
    it("should return the correct switch data after switch creation", async function () {
      const { sedn, owner } = await loadFixture(deploySednFixture);
      const testCadence = 10;
      const testFileUrl = "";

      const result = await sedn.connect(owner).mint(owner.address, testCadence, { value: ethers.utils.parseEther('6') });
      ...
      const [
        ...
        retFileUrl,
        retActive,
      ] = await sedn.getSwitchById(switchId);
    ...
    it("should emit a mint event", async function () {
      ...
      const testCadence = 10;
      await expect(
        await sedn.connect(owner).mint(owner.address, testCadence, { value: ethers.utils.parseEther('6') }),
      ).to.emit(sedn, "SwitchMinted");
    });
    it("claim funds", async function () {
      ...
      const paydayResult = await sedn.connect(owner).payday();
  ...
  describe("Switch Behaviour", function () {
    it("should be alive for the specified amount of blocks", async function () {
      ...
      const shouldBeAlive: boolean = await sedn.isAlive(switchId);
      ...
      const shouldBeDead: boolean = await sedn.isAlive(switchId);
  ...
  describe("Hearbeat Function", function () {
    it("should emit an event after heartbeat", async function () {
      ...
      await expect(await sedn.connect(owner).heartbeat(switchId)).to.emit(sedn, "HearBeatReceived");
    });
    it("should change the heartbeat block number after heartbeat", async function () {
      ...
      await sedn.connect(owner).heartbeat(switchId); // effect heartbeat
      const blockNumber = await ethers.provider.getBlockNumber();
      const switchInstance1 = await sedn.getSwitchForSwitchId(switchId);
  ...
  describe("Change Owner", function () {
    it("should emit an event after the change of owner", async function () {
      ...
      await expect(await sedn.connect(owner).changeOwner(switchId, newOwner)).to.emit(
        sedn,
        "OwnerChanged",
      );
    });
    it("should really change the owner of the switch in question", async function () {
      ...
      await sedn.connect(owner).changeOwner(switchId, newOwner);
      const switchInstance = await sedn.getSwitchForSwitchId(switchId);
  ...
  describe("Change Cadence", function () {
    it("should emit an event after the change of cadence", async function () {
      ...
      await expect(await sedn.connect(owner).changeCadence(switchId, newTestCadence)).to.emit(
        sedn,
        "ChangedCadence",
      );
    });
    it("should actually change the cadence of the switch in question", async function () {
      ...
      await sedn.connect(owner).changeCadence(switchId, newTestCadence);
      const switchInstance = await sedn.getSwitchForSwitchId(switchId);
  ...
  describe("Deactivate Switch", function () {
    it("should emit an event after the change of cadence", async function () {
      ...
      // deactivate
      await expect(await sedn.connect(owner).deactiviateSwitch(switchId)).to.emit(
        sedn,
        "SwitchDeactivated",
      );
    });
    it("should be marked as active = false after being deactivated", async function () {
      ...
      // deactivate
      await sedn.connect(owner).deactiviateSwitch(switchId);
      const shouldBeInactive: boolean = await sedn.isActive(switchId);
    ...
    it("should return an 'always' alive switch after being deactivated", async function () {
      ...
      const shouldStillBeAlive: boolean = await sedn.isAlive(switchId);
